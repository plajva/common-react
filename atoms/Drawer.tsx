 ...
 * @param param0 
 */
const Drawer: FunctionComponent<DrawerProps & React.HTMLAttributes<HTMLDivElement>> = ({ className, children, open: _open, setOpen: _setOpen, animTime: _animTime, fixed, maxWidth, ...props }) => {
			...
			<div className={className} {...props}>
				{(is_open || open) &&
					<>
						...
						<div className={classNameFind(s, `back`, fixed ? 'fixed' : '')} style={{ opacity: '0', transition: `opacity ${animTime}s` }} ref={back} onClick={() => { setOpen(false) }}></div>
						{/* The drawer */}
						<div className={classNameFind(s, `menu`, fixed ? 'fixed' : '')} ref={menu} style={{ maxWidth: maxWidth }}>
							{child_first}
							{hookContentData &&
				...
				<div className={classNameFind(s, `content`)}>
					{child_rest}
