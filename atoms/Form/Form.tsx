...
const Form = ({ initialState, validationSchema: schema, children, ...props }: FormProps) => {
	const [state, setState] = useState<FormState>({
		values:
			(schema &&
				((isZod(schema) && schema.safeParse(initialState)["data"]) || (isYup(schema) && schema.cast(initialState)))) ||
			initialState ||
			{},
		// errors: {},
		touched: {},
	...
	const context: FormContextI = {
		...
		setValue: (name, value) => {
			setState((state) => {
				...
				if (schema) {
					try {
						values = isZod(schema) ? schema.parse(values) : schema.validateSync(values, { abortEarly: false });
					} catch (_error) {
						errors = [];
								...
								errors.push(
									...e.issues.map((is) => {
										return {
											path: normalizeName(
												is.path.reduce(
													(p, pv) => p + "." + String(typeof pv === "number" ? `[${pv}]` : pv),
													""
												) as string
											),
											message: is.message,
...
export const FormNameProvider = (props: { children?: ReactNode; absolute?:boolean; name: string }) => {
	...
	return <FormNameContext.Provider value={props.absolute?props.name:name_context}>{props.children}</FormNameContext.Provider>;
};
