 ...
 */
const Apply: FunctionComponent<ApplyProps & React.HTMLAttributes<HTMLDivElement>> = ({ children, to, depth_max, ...props }) => {
	...
	const mergeProps = (child:ReactElement,depth): any => {
		...
		const m = (k, b, a) => {
			switch (k) {
				case 'className': return b + ' ' + a;
				default:
					switch (typeof b) {
						case 'string': return b + ' ' + a;
						default: console.error(`Can't handle key ${k}`); return b;
					}
			}
		}
