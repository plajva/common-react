...
const Stepper: FunctionComponent<StepperProps & React.HTMLAttributes<HTMLDivElement>> = (props) => {
	...
	let render_array = children.reduce<ReactNode[]>((arr, child, i) => {
		...
		let
			out_anim = done ? '' : `${s.out} ${animTime}s ${dirAndTiming} forwards`,
		...
		if ((i === nextStep || i === step)) {
			...
			if (React.isValidElement(child)) {
				if (nextStep === step) {
					...
					arr.push(React.cloneElement(child, { ref: nextRefCallback, style: { ...child.props.style, position: 'relative' }, key: i }));
					return arr;
				}
				if (i === nextStep)
					arr.push(React.cloneElement(child, { style: { ...child.props.style, animation: right ? in_anim : out_anim, transform: 'translateZ(0)' }, ref: nextRefCallback, key: i }))
				else {
					arr.push(React.cloneElement(child, { style: { ...child.props.style, animation: right ? out_anim : in_anim, transform: 'translateZ(0)' }, key: i }))
				}
			}
		}
		// else{
	...
	const steps = children.reduce<ReactNode[]>((arr, child, i) => { arr.push((React.isValidElement(child) && child.props.step) || i + 1); return arr }, []);
				...
				{steps.map((stepNode, i) =>
					<>
						<div className={classNameFind(s, `item`)} key={i}>
							<Button ripple_type="center" className={`circular ${propStep === i && 'primary-background'}`} onClick={() => { if (stepSet) stepSet(i) }}>{stepNode}</Button>
						</div>
						{i !== steps.length - 1 && <Divider className={classNameFind(s, `thinner item divider grow`)} key={i + 100} />}
					</>)}
			</div>
