...
import {FaHeart, FaHeartbeat, FaBriefcaseMedical, FaBookMedical, FaBaby, FaBandAid, FaCapsules, FaPlusSquare, FaPlusCircle} from 'react-icons/fa';
...
const buttons = (className:string, name:string,Component:any=Button) => {
	let append = (prefix:string) => {
		return className.split(' ').map(c => `${prefix?prefix+(c?'-':''):''}${c}`).join(' ');
	} 
	return (
		<div>
		{name?  <h3>{name}</h3> : ""}
		...
		>
			...
			&nbsp;
			<Component className={append(`primary`)} disabled>Disabled</Component>
		</div>
		</div>
	)
};
...
class Demo extends React.Component<DemoProps>{
	render(){
		const FieldFunc = (props) => {
			return (
				<Field name="name" label="Name" {...props}></Field>
			)
		}
			...
			<div style={{textAlign:'center', padding: '56px 5%'}}>
				...
				<Drawer>
					<div className="primary-background">
						<DrawerToggle><Button>Close</Button></DrawerToggle>
						Side Menu
					</div>
					<DrawerToggle><Button>Toggle Me</Button></DrawerToggle>
				</Drawer>
				<h2>Expander</h2>
				<Collapsible>
